...
class Rivets.View
  ...
  constructor: (@els, @models, @options = {}) ->
    ...
    for option in ['config', 'binders', 'formatters', 'adapters']
      @[option] = $.extend {}, Rivets[option], @options[option]

    @build()

  # Regular expression used to match binding attributes.
  bindingRegExp: => new RegExp "^#{@config.prefix}-"

  # Regular expression used to match component nodes.
  componentRegExp: => new RegExp "^#{@config.prefix.toUpperCase()}-"
  ...
  sync: => binding.sync() for binding in @bindings
